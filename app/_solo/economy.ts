 ...
 * 강화비. 구간마다 다른 규칙을 쓴다.
 *
 * - **+0~+5 안전 구간**: `ceil(카드 가치 × 9%)`
 * - **+6~+14 도박·트로피 구간**: `ceil(기본가 × 12%)`. 가치에 비례하지 않고 등급별 고정값
 *
 * 안전 구간을 11% 에서 9% 로 내렸다. 주인 피드백: "1~7카는 오히려 강화할수록 손해다.
 * 파괴 안 되는 6~7카까지도 강화 비용보다는 미세하게 가치가 더 높으면 좋겠다."
 * 재보니 사실이었다 — 11% 에서는 안전 구간 기대손익이 전 등급 −1.8~−3.6% 였고,
 * +6 까지 다 올려 팔면 레어 −49, 에픽 −131, 레전드 −371 이었다. +7 을 넘겨야 겨우
 * 본전이었으니 "올릴수록 손해"가 맞다.
 *
 * 9% 에서는 안전 구간이 0 언저리로 붙는다(레어 −0.8%~+0.2%). +0→+6 을 다 올려도
 * 손익이 기본가의 1% 안쪽이라(레어 −2, 레전드 +6) 벌지도 잃지도 못한다. 그 위
 * 도박 구간부터 남는다(레어 +7 에 +49).
 *
 * **여기서 규칙 하나가 바뀌었다.** 예전엔 "안전 구간 기대손익은 항상 음수"였다.
 * 위험 없는 반복 클릭이 이득이 되면 안 된다는 뜻이었는데, 부호로 못박으니 "올릴수록
 * 손해"가 따라왔다. 이제는 부호가 아니라 **절대액**으로 막는다: 안전 구간만 다 올려서는
 * 기본가의 몇 % 도 못 번다. 부호가 가끔 양수여도 절대액이 하찮으면 아무도 그걸로
 * 벌지 않는다. economy.test.ts 가 이 성질을 지킨다.
 *
