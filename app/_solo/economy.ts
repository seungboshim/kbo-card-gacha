 ...
 * 자리를 채우는 재료로서 갖는 값이 두드러지게 했다.
 *
 * 등급 간격은 아래로 갈수록 넓다: 3.00 → 2.67 → 2.38 → 2.11 배(기하평균 2.51).
 * 일부러 균등하게 안 맞췄다. 위쪽을 좁혀야 낮은 등급을 고강화한 카드가 갓 뽑은 높은
 * 등급을 따라잡는다(언커먼 +13 = 2,121 > 레전드 +0 = 2,000). 아래쪽을 넓힌 건 커먼과
 * 언커먼이 너무 붙으면 등급을 가르는 뜻이 없어져서다.
 */
