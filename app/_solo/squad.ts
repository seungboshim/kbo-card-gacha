...
// 형평이 안 맞는다.
//
// GK 의 y=90(예전 95)은 squad-field.tsx 필드 박스(aspect-[3/4])의 실측 기준이다.
// 95였을 때는 사진+이름+값 칸 전체 높이(버튼 기준 컨테이너의 약 15%)의 절반이
// -translate-y-1/2로 y=100% 아래까지 걸쳐, 값·★ 줄이 초록 필드 밖 검은 배경에 떠
// 있었다(브라우저 실측으로 확인). 90으로 당기면 버튼 아래쪽이 y≈97.5%로 필드 안에
// 들어오고 페널티 박스(PitchLines, 아래 15%인 y 85~100) 안에도 남는다.
//
// 3-4-3·3-5-2의 센터백 세 자리(CBL·CBC·CBR)는 예전 70·73·70(원래 80·83·80)으로
// 10 씩 끌어올렸다. GK 와 x=50 에서 겹치는 CBC 가 83 그대로면 버튼 높이(컨테이너의
// 약 15%)때문에 GK 칸과 세로로 겹쳐, 실제로 채워보면 CBC 카드가 GK 사진을 통째로
// 가렸다(브라우저 실측, 픽셀 단위로 확인) - 90(GK)과 20 가까이 벌려야 두 버튼이 안 닿는다.
export const FORMATION_SLOTS: Record<Formation, readonly Slot[]> = {
  "4-3-3": [
    slot("GK", "GK", "골키퍼", 50, 90, "골키퍼"),
  ...
  "3-4-3": [
    slot("GK", "GK", "골키퍼", 50, 90, "골키퍼"),
    slot("CBL", "DF", "센터백", 30, 70, "센터백"),
    slot("CBC", "DF", "센터백", 50, 73, "센터백"),
    slot("CBR", "DF", "센터백", 70, 70, "센터백"),
    slot("LM", "MF", "미드필더", 10, 50, "미드필더"),
    slot("CM1", "MF", "미드필더", 37, 54, "미드필더"),
    slot("CM2", "MF", "미드필더", 63, 54, "미드필더"),
    slot("RM", "MF", "미드필더", 90, 50, "미드필더"),
...
export const BASEBALL_SLOTS: readonly Slot[] = [
  slot("OF1", "OF", "외야수", 20, 17.0, "외야수"),
  slot("OF2", "OF", "외야수", 50, 11.3, "외야수"),
  slot("OF3", "OF", "외야수", 80, 17.0, "외야수"),
  slot("IF1", "IF", "내야수", 25, 59.4, "내야수"),
  slot("IF2", "IF", "내야수", 42, 53.8, "내야수"),
  slot("IF3", "IF", "내야수", 58, 53.8, "내야수"),
  slot("IF4", "IF", "내야수", 75, 59.4, "내야수"),
  slot("C", "C", "포수", 50, 82.1, "포수"),
  slot("DH", "DH", "지명타자", 88, 82.1), // exact 없음: 타자면 아무나
